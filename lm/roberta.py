import torch
from transformers import RobertaTokenizer, RobertaModel, RobertaForCausalLM
...
class RobertaLM(BaseLM):
    def load_model(self, model_path):
        #self.model = RobertaModel.from_pretrained(model_path)
        # Baseline RobertaModel doesn't appear to use labels in forward(), swapped for RobertaForCausalLM.
    ...
    def load_tokenizer(self,tokenizer_path):
        self.tokenizer = RobertaTokenizer.from_pretrained('FacebookAI/roberta-base' if tokenizer_path is None else tokenizer_path)
    ...
    def generate(self, input, k, mask_out=[], key=None, maxlen=512):
    ...
    def score(self, input, acts, maxlen=512):
        input_ids = self.sent2ids(input, maxlen=maxlen) if isinstance(input, str) else input
        input_len = len(input_ids)
        input_ids = torch.tensor([input_ids]).to(device)
        scores = []
